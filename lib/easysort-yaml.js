...
export default {
  ...
  activate(state) {
      ...
      return function(editor){
        ...
        if((editor.getGrammar()).name == "YAML"){
          buffer.onWillSave(function(){
            ...
            path = buffer.getPath();
            var config_paths = atom.config.get("easysort-yaml.autosort");
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'easysort-yaml:sort-keys': () => this.sort_keys()
    }));

    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'easysort-yaml:validate': () => this.validate_buffer(true)
    }));
  },
  ...
  sort_keys() {
    ...
    try {
      var doc = yaml_parser.safeLoad(editor.getText());
      editor.setText(yaml_parser.safeDump(doc, {
        sortKeys: true,
        lineWidth: atom.config.get('easysort-yaml.lineMaxLen'),
        noCompatMode: atom.config.get('easysort-yaml.noCompatibilityMode')
      }));
